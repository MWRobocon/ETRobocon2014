...
videoFileReader = vision.VideoFileReader('media/Lomography720p.mp4');
videoPlayer = vision.VideoPlayer();
videoWriter = vision.VideoFileWriter('media/Lomography720p_Tracked.mp4', 'FileFormat', 'MPEG4');
...
while ~isDone(videoFileReader)
    ...
    elseif size(visiblePoints, 1) >= 8 % atleast 8 points
        ...
        if xmin>0 && ymin>0 && scaleFactor*(xmax-xmin)+ xmin<c && scaleFactor*(ymax-ymin)+ ymin<r % if within image dimensions
            ...
            mode = 2; %mode = tracking with difficulty
        end
        ...
        mode = 3; %mode = unable to track, do setPoints() again
    end
