function [CentroidZ, CentroidS, visiblePoints] = detectTwoMinions(videoFrame, varargin)
...
% template = imread('media/Prince.png');
...
inlierThreshold = thresholdScale*max(w,h);
...
disp(num2str(size(visiblePoints, 1)));
CentroidZ(1,:) = mean(visiblePoints); % Sparse centroid of zumobot
CentroidZ(2,:) = mean(visiblePoints2); % Sparse centroid of zumobot

inlier = isInlier(single(CentroidZ(1,:)), visiblePoints, single(inlierThreshold));
