                 ...
                 const mxArray *prhs[])
{
	...
	if(hComm == INVALID_HANDLE_VALUE)
	{
		...
		FormatMessage(
			...
			0,
			NULL );
        mexPrintf(TEXT((LPCTSTR)lpMessageBuffer));
		LocalFree( lpMessageBuffer );
        mxFree(fname);
        mxFree(portname);
        mexErrMsgTxt("Cannot open com port.");
	}
                 ...
                 const mxArray *prhs[])
{
    ...
    if(ReadFile(handle->hComm, buf, tsize, &rsize, NULL) != 0) {
        plhs[0] = mxCreateDoubleMatrix(1, (int)rsize, mxREAL);
        for(cnt = 0; cnt < (signed)rsize; cnt++) {
            *((double *)mxGetPr(plhs[0]) + cnt) = *((unsigned char *)buf + cnt);
        }
